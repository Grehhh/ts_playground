const calculateWeeksPregnancy = (currentDate:string , birthDate:string ): number => {
    const end = new Date(birthDate);
    const start = new Date(currentDate);
  
    const timeDifference = end.getTime() - start.getTime(); // Obtain the diference in miliseconds
    const totalDaysDifference = timeDifference / (1000 * 3600 * 24); // miliseconds  in a day (1000ms*60secs*60 min*24hours*7days).
    // console.log('days diff: ' + totalDaysDifference)
    const weeks = Math.trunc((280 - totalDaysDifference) / 7); // truncate to obtain the integer part
    return weeks;
}
